...
- [x] **Brand name duplicated in eleven page titles** — _why:_ `SEO.tsx` appended " | Bústaðurinn.is" unless the title contained `|`, `—` or `–`. Eleven titles ended in " - Bústaðurinn.is" with a plain hyphen, so they rendered as "… - Bústaðurinn.is | Bústaðurinn.is", burning ~17 of a ~60-character budget. _done 2026-07-24:_ [SEO.tsx](src/components/SEO.tsx) now checks for the brand string itself. Fixes all eleven at once.
- [x] **Rewrite `/eiginleikar` title + description for intent + CTR** — _done 2026-07-24:_ title was 73 chars and truncated; now ~46. Description now leads with what the product does, incl. offline + árbók.
- [x] **Rewrite `/handbok` + `/handbok/bokunarkerfi` titles to match informational intent** — _done 2026-07-24:_ both dropped the abstract "sameignarhús" phrasing for the terms people actually search. `/handbok/bokunarkerfi` now leads with "Bókunarkerfi" (33 impressions, pos 29.7 — biggest non-brand query).
- [x] **Rewrite `/handbok/fjarmal` for the finance cluster** — _why:_ ranks page 1 for `hár rafmagnsreikningur` (pos 7.5, 22 impr), `sameiginlegur reikningur` (8.5), `sameiginlegur bankareikningur` (9.0), `utanumhald` (9.0) — all with 0 clicks. The old title ("Fjárhaldslausnir fyrir sameignarhús") answered none of them. _done 2026-07-24._
